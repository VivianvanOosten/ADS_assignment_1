---
title: "Assignment 1 - Wine Quality"
output: html_notebook
---

This notebook is created to answer all questions in Assignment 1 of the course Data Analaysis and Visualisation. It's made by Cedric Gevers (8051003) and Vivian van Oosten (6395929) of group 3. 

For this assignment, we looked through Kaggle for a suitable dataset. We found quite a few nice ones, but we were particularly interested in the wine dataset. It includes over 1500 wines with their chemical properties and a professionally determined quality between 3 and 8. One of the Kaggle challenges associated with this dataset was to predict the quality of the wine based on the chemical properties listed in the dataset. We decided to take up the challenge and see what our recently acquired data-skills could do for this dataset. Thus, our research question is: 
...
For this visualisation, we have filtered out one outlier of the volatile acidity that made the other points unnecessarily close together. Without losing too much data, we have a bigger and more clear plot. 
```{r}
# create a scatterplot from alcohol and volatile acidity
...
We try to filter out the missing data for our output variable, but there were none.
```{r}
# filter out all missing values from the quality column
...
Now we are going to split our wine dataset in train (50%), validation (30%) and test (20%).
```{r}
...
We have chosen to use a subset selection rather than regularisation methods, because we think that  limiting the number of columns we use but still allowing for large values in those coefficients will be better than limiting all coefficients. Because, if all coefficients are small, they will likely tend to predict only average qualities rather than also the outliers. Hopefully, with the subset selection, we can cater to the outliers as well. 

From here on, the best subset selection method will be implemented. 

We use the mse function from lab 4 to get the mean squared error of a certain formula. Note that we have added an argument that has NULL as a default to be able to use this one interchangably with the error function that takes the frequency into account.
```{r}
...
We source the generate_formulas file from Lab 4 to be able to implement the subset selection method. 
```{r}
...
In order to account for the skewed data we have and hopefuly be better able to predict whether a wine is really good or really bad, we have implemented a slightly different error function here. First we define a frequency column in the validation dataset. 
...
The inverted frequency is used to weigh the not frequent y's more. 
```{r}
...
Below we see that exactly the same formula came from this error function. 
This means that unfortunately, weighing the errors did not change the classification to something that says more about the high or low quality wines. 
```{r}
